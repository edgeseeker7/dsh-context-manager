...
export function historySearch(session, { query, limit = 10, beforeSeq, includeSelf = false }) {
  ...
  if (terms.length === 0) return { matches: [], scanned: session.seq, tier: null };
  ...
  for (let seq = scanFrom; seq >= 0; seq -= 1) {
    ...
    if (tier === 3) continue;
    candidates.push({ seq, type: event.type, tier, totalOccurrences, text, anchorIndex });
  }
  ...
  const matches = pool.slice(0, cap).map((candidate) => {
    const anchor = Math.max(candidate.anchorIndex, 0);
    const from = Math.max(0, anchor - SNIPPET_RADIUS_CHARS);
    const to = Math.min(candidate.text.length, anchor + SNIPPET_RADIUS_CHARS);
    return {
      seq: candidate.seq,
      type: candidate.type,
      snippet: `${from > 0 ? '…' : ''}${candidate.text.slice(from, to)}${to < candidate.text.length ? '…' : ''}`,
      offset: anchor,
      tier: candidate.tier,
    };
  });
  return { matches, scanned: session.seq, tier: bestTier === 3 ? null : bestTier };
}
