...
export function registerHistoryTools(agent, { historyMaxChars }) {
  agent.ctx.tools.register(
    defineTool({
      ...
      description:
        "Search this session's FULL history — including turns removed from your active context by a reset or compaction. Multi-term queries match events containing ALL terms (a whole phrase outranks term-AND; partial-term events only show when nothing matches all terms); a single short distinctive phrase is most precise. Results are ranked by hit count, density, and recency. Your own memory-tool calls AND the current turn's events are excluded by default (the current turn is already in your context — its query-echoing messages would flood the results; pass includeCurrentTurn to override). Checkpoint/summary events are marked — never treat compressed content as primary evidence; verify against the raw events. Each match gives seq + snippet + an offset you can pass to history_read to continue from the exact hit. Pass beforeSeq to search only below a known seq.",
      parameters: {
        query: {
          type: 'string',
          required: true,
