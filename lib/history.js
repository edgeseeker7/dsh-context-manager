 ...
 * best-tier pool.
 */
/** Cluster-cap bucket width (seqs): top results may not all come from one span. */
const CLUSTER_BUCKET_SEQS = 500;
/** Max results per cluster bucket. */
const CLUSTER_BUCKET_MAX = 2;
...
function* historySearchChunks(
  ...
  for (let seq = scanFrom; seq >= 0; seq -= 1) {
    ...
    // Self-echo: an event that (nearly) IS the query — the probe asking about
    // itself (dream trace had it topping its own results; its JSON splice is
    // excluded by type below). Demote it out of the phrase tier so
    // differently-worded real answers can surface. The ≤1.25× band catches
    // "query + a few chars" without touching short legitimate hits.
    const normalizedText = text.replace(/\s+/g, ' ').trim().toLowerCase();
    const normalizedNeedle = needle.replace(/\s+/g, ' ').trim();
    const echo = phraseIndex >= 0 && (normalizedText === normalizedNeedle || text.length <= needle.length * 1.25);
  ...
  // Cluster cap: at most CLUSTER_BUCKET_MAX results per CLUSTER_BUCKET_SEQS
  // span in the first pass, so one topic cluster cannot spend the whole
  // budget (dream trace: limit=6 once returned 6 hits from the same podcast
  // cluster); the second pass backfills from the overflow so small histories
  // (everything in one bucket) never starve.
  const bucketCount = new Map();
  const capped = [];
  const overflow = [];
  for (const candidate of pool) {
    const bucket = Math.floor(candidate.seq / CLUSTER_BUCKET_SEQS);
    const count = bucketCount.get(bucket) ?? 0;
    if (count >= CLUSTER_BUCKET_MAX) {
