...
import { JEV_RELEVANCE_THRESHOLD, jevRerank, jevSelectPages } from './jev.js';
...
export function installMalloc(
  agent,
  store,
  { resolveWindow, cachedWindow, nudgeEvery, measureTokens, suggestAnchors, logger, jevApiKey },
) {
  ...
  agent.ctx.on('agent/pre-step', async ({ turn, step }, next) => {
    ...
    try {
      ...
      if (step === 1) {
        ...
        // Echoes (the question matching itself) are never evidence — exclude
        // them from the pool before rerank and injection alike.
        let pool = strong ? result.matches.filter((m) => m.echo !== true) : [];
        ...
        if (jevApiKey !== undefined) {
          const verdicts = await jevRerank({
            ...
            request: userText,
            candidates: pool,
            onDegrade: (reason) => logger?.warn?.(`context-manager: jev rerank degraded to score gate: ${reason}`),
          });
          if (verdicts !== null) {
            ...
            logger?.info?.(`context-manager: jev rerank kept ${pool.length}/${before} hits`);
        ...
        let pagesSection = '';
        if (jevApiKey !== undefined) {
          const pages = sessionTopicMap(agent.session);
          const verdicts = await jevSelectPages({
            apiKey: jevApiKey,
            request: userText,
            pages,
            onDegrade: (reason) =>
              logger?.warn?.(`context-manager: jev page-select degraded to keyword-only: ${reason}`),
          });
          if (verdicts !== null) {
            ...
            logger?.info?.(`context-manager: jev page-select chose ${selected.length}/${pages.length} pages`);
