...
export class PinStore {
   ...
   */
  render(sessionId, cwd, capChars) {
    const task = this.load(this.taskFile(sessionId)).pins;
    const permanent = this.load(this.permanentFile(cwd)).pins;
    if (task.length === 0 && permanent.length === 0) return '';
    const cap = Number.isInteger(capChars) && capChars > 0 ? capChars : this.pinsMaxChars;
    const lines = [
    ...
    if (omitted.length > 0) {
      lines.push(
        `[vault overflow: ${omitted.length} pin${omitted.length > 1 ? 's' : ''} omitted (${omitted
          ...
          .join(
            ', ',
          )}) — over the ${cap}-char render cap for the current model's window. They are still allocated: see context_list, free with context_free.]`,
      );
    }
    return lines.join('\n');
  }
...
export function installMalloc(agent, store, { resolveWindow, cachedWindow, nudgeEvery, logger }) {
  ...
  agent.ctx.systemPrompt.section({
    ...
    text: () => {
      try {
        ...
        return store.render(sessionId, cwd, store.quotaChars(window));
      } catch (error) {
