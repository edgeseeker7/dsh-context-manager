...
export function installMalloc(
      ...
      if (step === 1) {
        ...
        if (jevApiKey !== undefined && pool.length > 0) {
          const verdicts = await jevRerank({
            ...
            request: userText,
            candidates: pool,
            onDegrade: (reason) => jevLog(`jev rerank degraded to score gate: ${reason}`, agent.session),
          });
          if (verdicts !== null) {
            ...
            jevLog(`jev rerank kept ${pool.length}/${before} hits`, agent.session);
          ...
          const verdicts =
            pages.length === 0
              ...
              : await jevSelectPages({
                  ...
                  request: userText,
                  pages,
                  onDegrade: (reason) => jevLog(`jev page-select degraded to keyword-only: ${reason}`, agent.session),
                });
          if (verdicts !== null) {
            ...
            jevLog(`jev page-select chose ${selected.length}/${pages.length} pages`, agent.session);
  ...
  const judgeNoteSpans = async () => {
    ...
    const verdicts = await jevSelectNoteSpans({
      ...
      spans,
      onDegrade: (reason) => jevLog(`jev notes degraded to the volume nudge: ${reason}`, agent.session),
    });
    ...
    jevLog(`jev notes kept ${worth.length}/${spans.length} spans`, agent.session);
    return worth;
  };
  ...
  const writeAutoNotes = async (spans) => {
    ...
    if (written.length > 0)
      jevLog(
        `jev notes auto-saved ${written.length} extract(s): ${written.map((entry) => entry.id).join(', ')}`,
        agent.session,
      );
    return written;
  };
