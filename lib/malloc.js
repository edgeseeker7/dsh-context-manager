...
import { historySearchAsync } from './history.js';
...
export function installMalloc(
  agent,
  store,
  { resolveWindow, cachedWindow, nudgeEvery, measureTokens, suggestAnchors, resetGate, logger },
) {
  // Post-reset auto-retrieval gate: the engine arms resetGate on /reset, and
  // the next model request gets one mechanical history_search injected. The
  // host-side answer to the dream-eval's 24.6% never-retrieve failure — the
  // checkpoint protocol text alone could not compel a search ("problem looks
  // self-contained"). One-shot: the gate is consumed on first use.
  if (resetGate !== undefined) {
    agent.ctx.on('agent/pre-step', async (_payload, next) => {
      const decision = await next();
      if (decision.kind === 'reject' || !resetGate.has(agent)) return decision;
      resetGate.delete(agent);
      try {
        ...
        const result = await historySearchAsync(agent.session, { query: userText, limit: 6 });
        const body =
          result.matches.length > 0
            ? result.matches.map((m) => `seq ${m.seq} [${m.type}] (offset ${m.offset}): ${m.snippet}`).join('\n')
            : `(no matches for: ${topTerms(userText, 6).join(' ') || userText.slice(0, 80).replace(/\n/g, ' ')})`;
        const mentionTail =
          Array.isArray(result.mentions) && result.mentions.length > 0
            ? `\nresults mention: ${result.mentions.join(' / ')}`
            : '';
          ...
          content: [
            {
              type: 'text',
              text:
                `[auto-retrieval after /reset — the checkpoint alone is not evidence; these are the log's best matches for your latest message]\n${body}${mentionTail}\n` +
                '— judge relevance yourself; search deeper with history_search({ query }) and history_read({ fromSeq, toSeq }); if this task is unrelated to prior work, ignore this and answer directly.',
            },
          ],
          source: { kind: 'plugin', plugin: 'dsh-context-manager', form: 'notice', summary: 'reset auto-retrieval' },
        });
        return { ...decision, messages: [...messages, notice] };
      } catch {
        return decision; // a failed auto-retrieval never blocks the turn
      }
    });
  }
