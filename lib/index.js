...
/**
 * Startup hygiene: remove crash-abandoned `.<pid>.tmp` write stubs older
 * than a day from the store directories (atomic-rename leftovers). Live
 * stores, quarantine files, and archives are NEVER touched — reporting
 * their counts is the operator's signal, deletion is their call.
 */
function sweepTmpFiles(logger) {
  const cutoff = Date.now() - 24 * 60 * 60 * 1000;
  for (const dir of [dshHomePath('context-manager', 'pins'), dshHomePath('context-manager', 'notes')]) {
    try {
      let removed = 0;
      for (const name of readdirSync(dir)) {
        ...
        try {
          ...
          /* a writer just claimed it — leave it alone */
        }
      }
      if (removed > 0) logger?.warn?.(`context-manager: swept ${removed} abandoned tmp file(s) from ${dir}`);
    } catch {
...
function resolveConfig(config = {}) {
  ...
  const resolved = {
    ...
    checkpointMaxChars: config.checkpointMaxChars ?? 8000,
    ...
    nudgeEvery: config.nudgeEvery ?? 20,
    suggestCount: config.suggestCount ?? 3,
    skillPromote: config.skillPromote ?? true,
  };
  for (const key of ['budgetHints', 'llmSummary', 'skillPromote']) {
  ...
  if (!Number.isInteger(resolved.checkpointMaxChars) || resolved.checkpointMaxChars < 2000)
    throw new Error('ContextManagerConfig: checkpointMaxChars must be an integer >= 2000');
...
export function apply(ctx, config = {}) {
  ...
  const notesStore = new NotesStore(resolved.notesMaxChars, { logger: ctx.logger });
  // pinMaxChars / pinsMaxChars are FALLBACK caps: the live gate is derived per
  // allocation from the context window of the model the calling agent routes to.
  ...
  const engine = new ContextResetEngine(isolated, {
    ...
    checkpointMaxChars: resolved.checkpointMaxChars,
    notesStore,
    pinStore,
    nudgeEvery: resolved.nudgeEvery,
    skillPromote: resolved.skillPromote,
    jevApiKey: resolved.jevApiKey,
