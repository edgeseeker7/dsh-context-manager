 ...
 * @module dsh-context-reset/command
 */
 ...
 */
export function registerResetCommand(ctx, engine) {
  const executeReset = async (invocation) => {
    ...
    try {
      ...
      return {
        kind: 'success',
        text: `Context window reset: ${result.shadowedSeqs.length} history items (~${result.shadowedTokenCount} tokens) left the active context un-summarized. Durable notes were re-injected; everything remains searchable with history_search.`,
  ...
  ctx.effect(function* () {
    ...
    yield ctx.commands.register({
      name: 'reset',
      description: 'Hard-reset the context window: no summary — durable notes re-injected, history stays searchable',
      handler,
    });
  }, 'context-reset: /reset lifecycle');
}
