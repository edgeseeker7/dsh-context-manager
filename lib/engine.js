...
/**
 * A bare UUID is almost never a pin-worthy fact: in these logs it is a session,
 * agent, tool-call or trace id, and recommending one wastes the nudge. Measured
 * in production — the candidate list offered three session UUIDs in a row.
 */
const BARE_UUID = /^[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/i;

/**
 * Turn raw anchor results into the pin candidates a nudge may name: ids first,
 * then paths, each long enough to be exact, not already recorded in the vault,
 * and never a bare UUID.
 *
 * @param {{ids: Array<[string, number]>, paths: Array<[string, number]>}} anchors
 * @param {string} pinned - concatenated text of the current vault.
 * @param {number} [limit]
 * @returns {string[]} at most `limit` candidates.
 */
export function pinCandidates({ ids, paths }, pinned, limit = 3) {
  return [...ids, ...paths]
    .map(([value]) => value)
    .filter((value) => value.length >= 12 && !pinned.includes(value) && !BARE_UUID.test(value))
    .slice(0, limit);
}
...
export class ContextResetEngine extends BasicCompactionEngine {
  ...
  constructor(ctx, config = {}) {
    ...
    this.skillPromote = skillPromote;
    this.jevApiKey = jevApiKey === '' ? undefined : jevApiKey;
  ...
  async summarize(input, agent, signal) {
    ...
    let notes = this.notesStore.read(session.id, { maxChars: notesBudget });
    const untruncatedNotes = this.notesStore.read(session.id, { maxChars: 10_000_000 });
    if (untruncatedNotes.length > notesBudget) {
      // A diary whose full text would blow the budget gets injected as an
      // INDEX (one line per note, all ids addressable via notes_read({ id }))
      // instead of a truncated dump — Claude Code MEMORY.md pattern: the
      // resident artifact is a bounded pointer table, bodies are on demand.
      // This is what stops reset artifacts from growing with the diary.
      notes = this.notesStore.renderIndex(this.notesStore.entries(session.id), { maxChars: notesBudget });
      notesHeader =
        '\n\n## Durable notes INDEX (persisted across resets — fetch any entry verbatim with notes_read({ id }))\n';
    }
    let text =
      fixed +
      notesHeader +
  ...
  mechanicalState(agent) {
    ...
    // Full-history topic map: the mechanical "menu" of what the shadowed log
    // contains. The mechanical state above only sees the tail window — dream
    // trace showed early-history topics invisible to the checkpoint, and the
    // model never searching for what it cannot imagine (16/65 zero-search).
    // Strata-sampled top terms need no LLM and cannot hallucinate.
    try {
      const lastSeq = session.seq - 1;
      if (lastSeq >= ANCHOR_SCAN_EVENTS) {
        const STRATA = 10;
        ...
        for (let s = 0; s < STRATA; s += 1) {
          ...
          for (let seq = start; seq <= end && corpus.length < 60000; seq += step) {
            ...
            if (event === undefined) continue;
            const text = readEventText(session, event);
            if (text.length > 0) corpus += `${text.slice(0, PER_EVENT_CHARS)}\n`;
          }
          const topics = topTerms(corpus, 3);
          if (topics.length > 0) spans.push(`seq ${start}..${end}: ${topics.join(' / ')}`);
        }
        if (spans.length > 0) {
          lines.push(
            `- History topic map (mechanical strata sample over the full shadowed log — the menu of what exists back there; run history_search around any span that looks relevant):\n  ${spans.join('\n  ')}`,
          );
        }
      }
    } catch (error) {
      lines.push(`- Topic map unavailable: ${error instanceof Error ? error.message : String(error)}`);
    }
    return lines;
  }
  ...
  installAgent(agent) {
    ...
    jevLog(
      this.jevApiKey === undefined
        ? 'jev disabled (no JEV_API_KEY) — score gate only'
        : 'jev enabled (rerank + page-select)',
      agent.session,
    );
    registerHistoryTools(agent, { historyMaxChars: this.historyMaxChars });
    registerNotesTools(agent, this.notesStore, {
      promote: {
        enabled: this.skillPromote,
  ...
  suggestAnchors(session) {
    try {
      ...
      return pinCandidates(extractor.result(), pinned);
    } catch {
      return [];
    }
  }
