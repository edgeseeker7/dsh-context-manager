...
/**
 * Pull the high-signal anchors (UUIDs, absolute paths, URLs) out of a text
 * corpus, ranked by frequency. Pure and exported for tests — this is the
 * mechanical complement to the LLM sketch: exact strings the sketch would
 * paraphrase away survive verbatim here.
 */
export function extractAnchors(text) {
  ...
  const top = (map, limit) => [...map.entries()].sort((a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : 1)).slice(0, limit);
  ...
  mechanicalState(agent) {
    ...
    try {
      ...
      lines.push(
        task.length === 0
          ...
          : `- Task pins cleared by this reset (${task.length}): ${task
              .map((pin) => `[${pin.handle}] ${pin.label}`)
              .join('; ')} — their verbatim text left the vault; re-derive facts with history_search when needed.`,
      );
    ...
    try {
      ...
      for (let seq = from; seq <= lastSeq; seq += 1) {
        ...
        corpus += `${readEventText(session, event).slice(0, ANCHOR_TEXT_CAP)}\n`;
      }
