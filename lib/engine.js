...
export class ContextResetEngine extends BasicCompactionEngine {
  ...
  constructor(ctx, config = {}) {
    ...
    // summaryConfig passes the optional summarization routing keys
    // (summarizationProvider/summarizationModel/maxTokens) through.
   ...
   * affordable — while notes and the retrieval tools stay authoritative.
   * A failed sketch call degrades to the pure hard cut, never to a failed
   * reset. The official transaction wraps the result in the checkpoint
   * framing and verifies it is smaller than the shadowed span.
   */
  async summarize(input, agent, signal) {
    ...
    if (this.llmSummary) {
      try {
        ...
        this.ctx.logger.warn(
          `context-reset: LLM sketch failed, falling back to pure cut: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    const sketchText =
    ...
    const text = [
      `CONTEXT WINDOW RESET #${prior + 1} (/reset, dsh-context-reset) — the earlier conversation left the active context; nothing was deleted.`,
      `The earlier conversation (${Math.max(input.messages.length - 1, 0)} replayed messages) remains FULLY recorded in this session's log. When a fact, path, decision or constraint matters, retrieve it instead of guessing or trusting the sketch below:`,
      ...
      ...(sketchText
        ? [
            '',
            '## Sketch of prior work (LLM-generated, UNVERIFIED — possibly incomplete or wrong; notes below and history_search are authoritative)',
            sketchText,
          ]
        : []),
      '',
    ...
    return {
      ...(sketch ?? { llmStreamCall: false, provider: 'context-reset', model: 'context-reset' }),
