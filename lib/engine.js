...
export class ContextResetEngine extends BasicCompactionEngine {
  ...
  /** @type {WeakSet<object>} agents whose next post-reset request gets one auto-retrieval injected. */
  resetGate = new WeakSet();
  ...
  async summarize(input, agent, signal) {
    ...
    // Arm the post-reset auto-retrieval gate: the next model request in this
    // session gets one mechanical history_search injected (the host-side
    // answer to the 24.6% never-retrieve failure — the checkpoint's retrieval
    // protocol alone could not compel it).
    this.resetGate.add(agent);
    return {
