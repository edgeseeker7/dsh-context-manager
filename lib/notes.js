...
export class NotesStore {
  ...
  /**
   * Every session diary in the store directory, newest-write first. Cheap
   * scan: line counts and the last parseable timestamp, no full parse —
   * this is the discoverability answer for "a NEW session starts with an
   * empty diary": the old diaries exist, and now they can be found.
   */
  sessionsIndex() {
      ...
      try {
        ...
        for (let lineIndex = lines.length - 1; lineIndex >= 0; lineIndex -= 1) {
          try {
            ...
            if (typeof parsed?.ts === 'string' && parsed.ts.length >= 10) {
              lastTs = parsed.ts.slice(0, 10);
              break;
            }
          } catch {
            /* torn tail line — keep looking upward */
          }
        }
        index.push({
  ...
  /**
   * Render the whole version chain around one note as an indented tree:
   * roots (notes superseding nothing) down to every active endpoint, the
   * queried note marked. The chain stops being a by-id fragment view and
   * becomes an object the model can see whole. Capped at 30 nodes; cycles
   * break at the first repeat.
   */
  chainTree(entries, id) {
    const byId = new Map(entries.map((entry) => [entry.id, entry]));
    if (!byId.has(id)) return '';
    const { superseded } = this.chainViews(entries);
    const successorsOf = (entryId) =>
      entries.filter((entry) => entry.supersedes.includes(entryId)).map((entry) => entry.id);
    // Find the connected component (edges walk both ways from the target).
    const component = new Set([id]);
    const queue = [id];
    while (queue.length > 0 && component.size < 200) {
      const current = queue.shift();
      const neighbors = [...(byId.get(current)?.supersedes ?? []), ...successorsOf(current)];
      for (const next of neighbors) {
        if (!byId.has(next) || component.has(next)) continue;
        component.add(next);
        queue.push(next);
      }
    }
    // Roots: component members that supersede nothing (within the component).
    const roots = [...component].filter((entryId) =>
      (byId.get(entryId)?.supersedes ?? []).every((target) => !component.has(target)),
    );
    const lines = [];
    const visited = new Set();
    const walk = (entryId, depth) => {
      if (lines.length >= 30) return;
      if (visited.has(entryId)) {
        lines.push(`${'  '.repeat(depth)}[${entryId}] (↩ cycle)`);
        return;
      }
      visited.add(entryId);
      const entry = byId.get(entryId);
      const ultimates = superseded.get(entryId);
      const status = ultimates !== undefined ? `superseded by ${ultimates.join(', ')}` : 'active';
      const preview = entry.text.replace(/\s+/g, ' ').slice(0, 60);
      const stamp = entry.ts.length >= 10 ? ` ${entry.ts.slice(0, 10)}` : '';
      lines.push(
        `${'  '.repeat(depth)}[${entryId}${stamp}] ${preview}${entry.text.length > 60 ? '…' : ''} — ${status}${entryId === id ? ' ←' : ''}`,
      );
      for (const next of successorsOf(entryId)) {
        if (component.has(next)) walk(next, depth + 1);
      }
    };
    for (const root of roots.sort()) walk(root, 0);
    const total = component.size;
    return `version chain around ${id} (${total} note${total > 1 ? 's' : ''}):\n${lines.join('\n')}${total > 30 ? `\n… (${total - 30} more nodes)` : ''}`;
  }

  /**
   * One note plus its chain status, for the skill-promoter: { entry,
   * superseded } or null when the id does not exist.
   */
  inspect(sessionId, id) {
    ...
    const { superseded } = this.chainViews(entries);
    return { entry, superseded: superseded.has(id) };
  }

  /** Read the diary: load entries from disk, then render the derived view.
   * `session` reads ANOTHER session's diary (read-only semantics: migration
   * seeding still runs, but nothing is ever appended cross-session). */
  read(sessionId, opts = {}) {
    if (opts?.listSessions === true) {
      const index = this.sessionsIndex();
      if (index.length === 0) return '(no diaries in this workspace yet)';
      return `${index
        .map(
          (entry) =>
            `${entry.sessionId} — ${entry.notes} notes, ${entry.bytes} bytes${entry.lastTs ? `, last ${entry.lastTs}` : ''}`,
        )
        .join('\n')}\n— read one with notes_read({ session: "<sessionId>" })`;
    }
    const target =
      typeof opts?.session === 'string' && opts.session.trim().length > 0 ? opts.session.trim() : sessionId;
    if (typeof opts?.chain === 'string' && opts.chain.trim().length > 0) {
      const tree = this.chainTree(this.entries(target), opts.chain.trim());
      return tree.length > 0 ? tree : `(no note ${opts.chain.trim()} in this diary)`;
    }
    return this.renderView(this.entries(target), opts);
  }
   ...
   */
  renderView(allEntries, { tag, id, includeSuperseded = false, listTags = false, maxChars } = {}) {
    if (listTags) {
    ...
    const budget = Number.isInteger(maxChars) && maxChars > 0 ? maxChars : this.maxChars;
    let total = blocks.reduce((sum, block) => sum + block.line.length, 0) + Math.max(0, blocks.length - 1) * 2;
    if (total <= budget) return blocks.map((block) => block.line).join('\n\n');
    const droppedIds = [];
    let droppedChars = 0;
    ...
    const excess = content.length - budget;
    return `[older notes truncated — ${droppedChars + excess} earlier chars not shown]${suffix}\n${content.slice(-budget)}`;
  }

  /**
   * Index form of the diary: one line per active note (id + tags + 80-char
   * preview), budget-capped. The checkpoint and nudges inject THIS when the
   * full text would blow the budget — every note stays addressable via
   * notes_read({ id }) while the injected block has a hard ceiling, however
   * large the diary grows (Claude Code MEMORY.md pattern: index resident,
   * bodies on demand).
   */
  renderIndex(allEntries, { maxChars = 3000 } = {}) {
    ...
    for (const entry of allEntries) {
      ...
      const tags = entry.tags.length > 0 ? ` ${entry.tags.map((t) => `#${t}`).join(' ')}` : '';
      const preview = entry.text.replace(/\s+/g, ' ').slice(0, 80);
      lines.push(`[${entry.id}${stamp ? ` ${stamp}` : ''}${tags}] ${preview}${entry.text.length > 80 ? '…' : ''}`);
    }
    const kept = [];
    let total = 0;
    for (let i = lines.length - 1; i >= 0; i -= 1) {
      if (total + lines[i].length + 1 > maxChars) continue;
    ...
    return (
      kept.join('\n') +
      (dropped > 0
        ? `\n… ${dropped} older notes not indexed here — find them with notes_read({ listTags: true }) or notes_read({ id })`
        : '')
    );
  }
  ...
  async append(sessionId, text, { tags, supersedes, sourceSeq } = {}) {
    ...
    return await withFileLock(
      ...
      () => {
        ...
        const result = { accepted: true, id, fileChars: statSync(path).size, viewChars: view.length };
        if (text.length > 2000) {
          // A single giant note defeats the injected view (the tail budget
          // fits only it) — advisory only, never a rejection.
          result.advisory = `note is ${text.length} chars — consider splitting into 2-3 smaller notes so the injected index stays informative`;
        }
        return result;
      },
      { logger: this.logger },
 ...
 * `options.promote` (optional) wires the skill-promoter: { enabled, jevApiKey,
 * logger } — by-id fetches are counted workspace-wide and hot procedural
 * notes earn a crystallization hint (see promote.js). */
export function registerNotesTools(agent, store, options = {}) {
  ...
  agent.ctx.tools.register(
    defineTool({
      ...
      description:
        'Read your durable notes that persist across context resets and compactions. Notes are per-session; listSessions finds diaries from OTHER sessions in this workspace, and session reads one of them (read-only). By default shows active notes plus one-line audit entries for superseded ones; listTags enumerates every bucket you have invented with its active-note count, tag reads one bucket, id fetches one note verbatim (including its chain status — this bypasses the budget, so it is also the way back to notes the truncation marker lists as dropped), chain renders the whole version tree around one note, includeSuperseded expands folded notes back to full text.',
      parameters: {
        listSessions: {
          type: 'boolean',
          description:
            'When true, list every session diary in this workspace (session id, note count, last write) — the way to find notes from other conversations.',
        },
        chain: {
          type: 'string',
          description:
            'Optional note id (e.g. "n7") — render the whole version chain around it as a tree: roots down to every active successor, statuses included.',
        },
        session: {
          type: 'string',
          description: "Optional session id — read that session's diary (read-only) instead of this one.",
        },
        listTags: {
          type: 'boolean',
      ...
      output: {
        ...
        render: (_args, value) => [
          { type: 'text', text: `${value.notes.length > 0 ? value.notes : '(no notes yet)'}${value.hint ?? ''}` },
        ],
      },
      isConcurrencySafe: () => true,
      execute: async (args) => {
        const notes = store.read(agent.session.id, {
          listSessions: args?.listSessions === true,
        ...
        if (
          ...
          // Encounter-time signpost: an empty diary in a workspace that HAS
          // older diaries is a fork in the road, not a dead end — say so at
          // exactly the moment the model hits it.
          const others = store
            .sessionsIndex()
            .filter((entry) => entry.sessionId !== agent.session.id && entry.notes > 0);
          if (others.length === 0) {
            out = { notes };
          } else {
            ...
            out = {
              notes:
                `(no notes yet in THIS session — but ${others.length} other session${others.length > 1 ? 's' : ''} in this workspace ` +
                `${others.length > 1 ? 'have' : 'has'} diaries; newest: ${newest.sessionId}, ${newest.notes} notes${newest.lastTs ? `, last ${newest.lastTs}` : ''}. ` +
                `List them with notes_read({ listSessions: true }), read one with notes_read({ session: "<id>" }).)`,
            };
          }
        }
        // Skill-promoter hook: by-id fetches are the "this note is hot"
        // signal. Count, gate, and (rarely) attach a crystallization hint.
        // Any failure here must never break the read itself.
        ...
        if (
          ...
          try {
            const { recordFetch, maybeSuggest } = await import('./promote.js');
            const target =
              typeof args?.session === 'string' && args.session.trim().length > 0
                ? args.session.trim()
                : agent.session.id;
            const found = store.inspect(target, args.id.trim());
            if (found !== null) {
              const key = `${target}::${found.entry.id}`;
              await recordFetch(key, { logger: promote.logger });
              const hint = await maybeSuggest({
                key,
                note: found.entry,
                superseded: found.superseded,
                jevApiKey: promote.jevApiKey,
                session: agent.session,
                logger: promote.logger,
              });
              if (hint !== undefined) out.hint = hint;
            }
          } catch (error) {
            promote.logger?.warn?.(
              `context-manager: skill-promoter failed (read unaffected): ${error instanceof Error ? error.message : String(error)}`,
            );
          }
        }
        return out;
