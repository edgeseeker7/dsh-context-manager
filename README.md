...
| `notes_read([listSessions, session, listTags, id, tag, includeSuperseded])` | Read the diary (per-session): `listSessions` discovers other sessions' diaries in this workspace, `session` reads one (read-only); `listTags` enumerates buckets, `id` fetches one note verbatim (the escape hatch for truncated-away notes), `tag` reads one bucket, `includeSuperseded` expands folded notes. An empty diary in a workspace with older diaries answers with a signpost to them. |
